...
def chat():
    ...
    try:
        response = requests.request(
            "POST", reqUrl, json=payload, headers=headersList, timeout=30
        )
        ...
        app.logger.error("Connection error: Unable to reach PrivateGPT service.")
        return (
            jsonify(
                {
                    "error": "Unable to connect to the PrivateGPT service. Please try again later."
                }
            ),
            503,
        )
    except requests.exceptions.Timeout:
        app.logger.error("Request to PrivateGPT service timed out.")
        return jsonify({"error": "The request timed out. Please try again later."}), 504
    except requests.exceptions.HTTPError as e:
        app.logger.error("HTTP error: %s", str(e))
        return (
            jsonify({"error": f"HTTP Error: {e.response.status_code}"}),
...
def list_ingested_files():
    try:
        reqUrl = "http://localhost:8001/v1/ingest/list"
...
def ingest_file():
    ...
    if file:
        try:
            reqUrl = "http://localhost:8001/v1/ingest/file"
